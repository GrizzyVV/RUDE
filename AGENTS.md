...
| `InspectMesh` | `AssetPath` | **+ `materials[]` (2026-09-06):** per slot the instance path and the texture its `Diffuse` resolved to (`unbound` names a checkered geometry; the district's bind report only counts). The numbers behind an imported mesh: render bounds, LOD0 vertex extents, triangle count, collision primitive counts and the farthest primitive. The instrument that measured a 7 m tarp imported as a 260 m sheet (2026-09-05) | agent |
