...
| `ExportYddBinary` | `SkeletalMeshAssetPaths, DrawableNames, OutYddPath, Options` | GDD "custom clothing": skinned USkeletalMeshes (ImportPed output, or a Blender garment whose bones are NAMED like the ped rig) written as a binary `.ydd` drawable dictionary (RSC7 v165, entries in ascending hash order, one High model per entry, `ped` shader with Diffuse/Normal/Specular stubs from the material instance, bone-id table = identity over the rig, 4 influences per vertex summing to 255). Comma lists pair asset i with name i. Counts `meshBonesUnmapped/influencesUnmapped/uv1Dropped/texturesMissing`; refuses rigs over 255 bones. Gate: a_m_m_business_01 uppr 2,213 verts / 2,240 tris / max bone 104 = corpus 19/19 (maintainer lane `ydd_writer`). ❓ in-game load is Matt's test - the game also needs the matching ytd + a ped variation ymt row | agent |
