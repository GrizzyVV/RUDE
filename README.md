...
| 🔴 **No *constructive* fragment (`.yft`) writer; no water lane; navmesh and particles are read-only.** | Vehicles and weapons import and can be posed, fitted and reskinned, and an EXISTING fragment can be cloned and have its geometry replaced (byte-checked, with the drawable's bounds recomputed) — but authoring one that has no original needs a constructive writer that does not exist. `DebugDrawNavmesh` draws a `.ynv`, `ImportParticleEffects` reads a `.ypt`; neither writes. FiveM water comes from `water.xml`, which RUDE does not read or write. |
| 🟡 **Ped variation rows CAN be added — this used to say they could not.** | `AddPedDrawable` + `ExportPedVariationYmt` write the variation table: 4 peds export byte-identically and an added drawable changes exactly one component. Replacing a ped's own dictionary (`ExportPedReplace`) is still the other route. ❓ Neither has been loaded in-game yet. |
